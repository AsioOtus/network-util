...
class GeneralDelegate <RequestType: Request, ResponseType: Response, ContentType, ErrorType: NetworkUtilError>: RequestDelegate {
    let name: String
    ...
    func request (_ requestInfo: RequestInfo) throws -> RequestType { try requestHandler(requestInfo) }
    
    func urlSession (_ request: RequestType, _ requestInfo: RequestInfo) throws -> URLSession { try urlSessionHandler(request, requestInfo) }
    func urlRequest (_ request: RequestType, _ requestInfo: RequestInfo) throws -> URLRequest { try urlRequestHandler(request, requestInfo) }
    
    func response (_ data: Data, _ urlResponse: URLResponse, _ requestInfo: RequestInfo) throws -> ResponseType { try responseHandler(data, urlResponse, requestInfo) }
    func content (_ response: ResponseType, _ requestInfo: RequestInfo) throws -> ContentType { try contentHandler(response, requestInfo) }
    
    func error (_ error: NetworkError, _ requestInfo: RequestInfo) -> ErrorType { errorHandler(error, requestInfo) }
}

extension GeneralDelegate where ErrorType == NetworkError {
    convenience init (
        name: String,
        request: @escaping (RequestInfo) -> RequestType,
        content: @escaping (ResponseType, RequestInfo) -> ContentType,
